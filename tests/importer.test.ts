...
async function setup() {
  ...
  const files={teams:teamsFixture,sources:{ratings:{access:'approved',authorizationReference:'TEST FIXTURE ONLY'}},standings:{snapshots:[]},ratings:{snapshots:[]},status:{standings:{state:'pending',lastSuccessAt:null},ratings:{state:'pending',lastSuccessAt:null}}};
  for(const [name,value] of Object.entries(files))await writeFile(join(dir,`${name}.json`),JSON.stringify(value));
  return dir;
}
test('failed import preserves the last valid file; subsequent recovery succeeds',async()=>{
  ...
  try {
    ...
    const next=standingsFixture();next.rows[0].gp=1;next.rows[0].w=1;next.rows[0].points=2;
...
test('MHR permission gate applies to file imports too',async()=>{
  const dir=await setup();
  try{
    await writeFile(join(dir,'sources.json'),JSON.stringify({ratings:{access:'pending',authorizationReference:null}}));
    await assert.rejects(()=>importSnapshot('ratings',ratingsFixture('2026-09-23'),dir,new Date('2026-10-30')),/authorization/);
...
test('feed rejects blocked access without retrying or bypassing, and refuses HTML',async()=>{
  let calls=0;
  const blocked=(async()=>{calls++;return new Response('Forbidden',{status:403});}) as typeof fetch;
  await assert.rejects(()=>fetchJSON('https://example.com',undefined,blocked),/403/);assert.equal(calls,1);
  const html=(async()=>new Response('<html>challenge</html>',{headers:{'content-type':'text/html'}})) as typeof fetch;
