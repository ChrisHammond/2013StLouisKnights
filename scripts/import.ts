...
if (scheduled && (now.slice(0,10) < '2026-08-01' || now.slice(0,10) > '2027-07-31')) {
...
} else if (kind === 'ratings' && (sources.ratings.access !== 'approved' || !sources.ratings.authorizationReference)) {
  ...
  try {
    const input = file ? JSON.parse(await readFile(file,'utf8')) : await fetchJSON(url!,process.env[`${prefix}_FEED_TOKEN`]);
    const changed = await importSnapshot(kind,input);
    await recordStatus(kind,{state:file ? 'manual' : 'ok',lastAttemptAt:now,lastSuccessAt:now,message:changed ? 'Validated source observation saved.' : 'Source checked; no new revision.'});
    ...
    await recordStatus(kind,{state:'error',lastAttemptAt:now,message:'The latest update failed validation or source access. The last valid observation is retained.'});
