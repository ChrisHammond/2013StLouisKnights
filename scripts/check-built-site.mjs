...
for(const file of htmlFiles){
  ...
  if(/TEST ONLY|fixture-knights|fixture-opponent|TEST FIXTURE/.test(html))throw new Error(`Test data leaked into ${file}`);
  for(const match of html.matchAll(/(?:href|src)="(\/[^"#?]*)[^\"]*"/g)){
    const target=match[1];if(target.startsWith('//'))continue;
    await access(join('dist',target.endsWith('/')?`${target}index.html`:target));links++;
  }
  if(!html.includes('<h1'))throw new Error(`Missing page heading: ${file}`);
}
console.log(`Verified ${htmlFiles.length} built pages and ${links} local links/assets; no test fixtures leaked.`);
