...
export const teamSchema = z
  .object({
    ...
    abbr: z.string().min(1),
    gamesheetId: z.string().regex(/^\d+$/),
    mhrId: z.string().regex(/^\d+$/),
    leagueUrl: httpsUrl,
    clubUrl: httpsUrl,
    clubLinkType: z.enum(['club', 'team']),
    mhrUrl: httpsUrl,
    verifiedAt: z.iso.date(),
    clubNote: z.string().optional(),
  })
  .strict();
...
export const ratingRowSchema = z
  .object({
    teamId: id,
    rating: z.number().positive().max(200).nullable(),
...
export function validateSnapshot(kind: Kind, input: unknown, teams: Team[], now = new Date()): Snapshot {
  ...
  if (
    ...
    actual.some((team) => !expected.has(team))
  )
    throw new Error(
      'Snapshot must contain every division team exactly once; use null for unavailable ratings',
    );
  ...
  if (
    collected > now.getTime() + 60_000 ||
    observed > collected ||
    (published !== null && published > observed)
  )
    throw new Error('Invalid chronology: publication ≤ observation ≤ collection ≤ now');
  if (snapshot.source === 'myhockeyrankings') {
    if (
      snapshot.releaseDate < '2026-08-01' ||
      snapshot.releaseDate > '2027-07-31' ||
      snapshot.releaseDate > snapshot.observedAt.slice(0, 10)
    )
      throw new Error('Rating release is outside the season or in the future');
    for (const row of snapshot.rows)
      if (row.rating === null && row.rank !== null) throw new Error('An unrated team cannot have a rank');
  } else {
    const sourceUrl = new URL(snapshot.sourceUrl);
    if (
      sourceUrl.hostname !== 'gamesheetstats.com' ||
      sourceUrl.pathname !== '/seasons/15220/standings' ||
      sourceUrl.searchParams.get('filter[division]') !== '81589' ||
      sourceUrl.searchParams.get('filter[type]') !== 'regular_season'
    )
