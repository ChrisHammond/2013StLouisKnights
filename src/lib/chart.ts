...
export function chartModel(releases: RatingsSnapshot[], selected: string[]) {
  const values = releases.flatMap((release) =>
    release.rows
      .filter((row) => selected.includes(row.teamId) && row.rating !== null)
      .map((row) => row.rating!),
  );
  ...
  const series = selected.map((teamId) => {
    ...
    releases.forEach((release, index) => {
      ...
      if (
        row?.rating == null ||
        (prior &&
          (Date.parse(release.releaseDate) - Date.parse(prior.releaseDate) !== 7 * 86400000 ||
            prior.category !== release.category))
      ) {
      ...
      if (row?.rating != null)
        segment.push({
          ...
          value: row.rating,
          x: x(release.releaseDate),
          y: y(row.rating),
          delta: weeklyChange(releases, teamId, index),
          rank: row.rank,
