...
export function effectiveTime(snapshot: Snapshot): string {
  return snapshot.source === 'myhockeyrankings'
    ? snapshot.releaseDate
    : (snapshot.sourcePublishedAt ?? snapshot.observedAt);
}
...
export function appendSnapshot<T extends Snapshot>(
  store: Store<T>,
  snapshot: T,
): { store: Store<T>; changed: boolean } {
  // Only compare the latest revision for a source date: a reverted correction is meaningful.
  const prior = [...store.snapshots]
    .reverse()
    .find((s) =>
      snapshot.source === 'myhockeyrankings'
...
export function currentSnapshot<T extends Snapshot>(snapshots: T[]): T | undefined {
  return snapshots.reduce<T | undefined>(
    (latest, item) => (!latest || effectiveTime(item) >= effectiveTime(latest) ? item : latest),
    undefined,
  );
}
...
export function weeklyChange(
  ...
  if (
    Date.parse(current.releaseDate) - Date.parse(previous.releaseDate) !== 7 * 86400000 ||
...
export function formatDate(value: string | null): string {
  ...
  return new Intl.DateTimeFormat('en-US', {
    ...
    day: 'numeric',
    year: 'numeric',
    timeZone: value.length === 10 ? 'UTC' : 'America/Chicago',
