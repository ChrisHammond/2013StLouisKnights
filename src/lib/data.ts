...
import { validateTeams, validateSnapshot, type StandingsSnapshot, type RatingsSnapshot, type SourceStatus } from './schema';
...
export const standings = currentSnapshot(standingsData.snapshots.map(s => validateSnapshot('standings',s,teams) as StandingsSnapshot));
export const releases = ratingReleases(ratingsData.snapshots.map(s => validateSnapshot('ratings',s,teams) as RatingsSnapshot));
